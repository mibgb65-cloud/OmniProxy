...
- **Codex quota-window auto-activation**: Global Settings now provides an explicit opt-in switch. When enabled, OmniProxy sends one minimal Codex request if the expected five-hour or weekly window has not started, then refreshes usage; accounts with active windows are not called again.
- **Codex activation observability**: Enabling the switch now starts an immediate Codex account scan. Each Codex account card on the Quota page also exposes **Check and activate**, while start, success, skip, and failure events are persisted in request history.
